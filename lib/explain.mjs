...
export function explainFailure({ status, body, configMissing = [] } = {}) {
  ...
  if (status === 401 || status === 403) {
    // The data plane's own credential rejections carry a string `error` field
    // ("Missing Authorization header", "Missing Kavros credentials"). An
    // upstream that answered 401/403 AFTER policy passed looks different:
    // Stripe nests `error` as an object, other APIs send raw text or their own
    // shapes. Distinguish them so the footer never tells an operator whose
    // request physically reached the target that "nothing was sent onward".
    // Known edge: an upstream that happens to return a flat string `error`
    // could be misread as a data-plane rejection — acceptable while the data
    // plane does not yet tag which hop produced the status.
    const dataPlaneCredentialRejection =
      typeof body?.error === "string" &&
      /missing |unauthorized|forbidden/i.test(body.error);

    if (dataPlaneCredentialRejection) {
      return {
        headline: "Your workload identity was rejected.",
        egress:
          "The data plane refused the credentials before any policy evaluation — nothing was sent onward.",
        next: [
    ...
    return {
      headline: `The upstream target answered HTTP ${status}.`,
      egress:
        "Policy checks passed and the request reached the target — the target itself refused it, usually for a missing or invalid API credential. This is not a Kavros denial.",
      next: [
        "Your policy and identity were accepted — the refusal came from the target service.",
        'Provide the credential the target requires, e.g. --header "Authorization: Bearer <token>" (or its documented auth scheme).',
        "Some APIs also require a User-Agent header — the GitHub API returns 403 without one.",
      ],
    };
  }
