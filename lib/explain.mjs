...
export function explainFailure({ status, body, configMissing = [] } = {}) {
  if (configMissing.length > 0) {
    return {
      ...
      next: [
        ...
        "Then run `enclavia doctor` to confirm connectivity.",
      ],
    };
  }
  ...
  const reason =
    typeof body === "object" && body !== null
      ...
      : typeof body === "string" && body
        ? body
        : null;
  const blocked = typeof body === "object" && body !== null && body.error === "Blocked by Enclavia";
  ...
  if (blocked) {
    return {
      ...
      next: [
        ...
        "Run `enclavia doctor` to rule out an identity problem (halted or revoked workloads also block).",
      ],
    };
  }
  ...
  if (status === 401 || status === 403) {
    return {
      ...
      next: [
        "Check that ENCLAVIA_API_KEY and ENCLAVIA_AGENT_ID belong to the same registered workload.",
  ...
  if (status === 404) {
    return {
      ...
      next: [
        "Confirm ENCLAVIA_API_URL points at the data-plane egress endpoint (…/api/agent/egress), not the control plane UI.",
        "Confirm ENCLAVIA_AGENT_ID matches a workload registered on this deployment.",
      ],
    };
  }
  ...
  if (status !== undefined) {
    return {
      ...
      next: ["Run `enclavia doctor` to verify connectivity and identity, then retry."],
    };
  }
