// Signed workflow-bundle verification for the Enclavia CLI. Pure functions,
...
// so "trust us" becomes "run enclavia verify-bundle" locally, including on
// machines that never talk to a deployment.

import crypto from "node:crypto";

const BUNDLE_FORMAT = "enclavia-workflow-bundle";
 ...
 *   Falls back to ENCLAVIA_POLICY_PUBLIC_KEY / CP_POLICY_PUBLIC_KEY env vars.
...
export function verifyBundle(signed, options = {}) {
  ...
  if (signed.algorithm !== "ed25519") {
    return {
      ok: false,
      reason: `Unsupported signature algorithm: ${signed.algorithm ?? "none"}. Enclavia bundles are Ed25519-signed; this file may not be a bundle export.`,
    };
  }
  const publicKeyBase64 = options.publicKey ?? process.env.ENCLAVIA_POLICY_PUBLIC_KEY ?? process.env.CP_POLICY_PUBLIC_KEY;
  if (!publicKeyBase64) {
    return {
      ok: false,
      reason: "No trusted public key supplied. Pass --public-key <base64> or set ENCLAVIA_POLICY_PUBLIC_KEY (the CP_POLICY_PUBLIC_KEY value from provisioning).",
    };
  }
  ...
  if (
    ...
    return { ok: false, reason: "Not a valid Enclavia workflow bundle (missing format markers or workflow name)." };
  }
