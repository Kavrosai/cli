// Pure argument and configuration helpers for the Enclavia CLI. Kept free of
...
export function parseArgs(argv) {
  ...
  for (let index = 0; index < argv.length; index += 1) {
    ...
    else if (value === "--action" || value === "--target" || value === "--content" || value === "--header" || value === "--timeout") {
...
export function config() {
  return {
    url: process.env.ENCLAVIA_API_URL,
    apiKey: process.env.ENCLAVIA_API_KEY,
    agentId: process.env.ENCLAVIA_AGENT_ID,
  };
}
...
export function requireConfig(values = config()) {
  const missing = Object.entries(values)
    .filter(([, value]) => !value)
    .map(([key]) => key === "url" ? "ENCLAVIA_API_URL" : key === "apiKey" ? "ENCLAVIA_API_KEY" : "ENCLAVIA_AGENT_ID");
  if (missing.length > 0) {
...
// workload credentials (x-enclavia-*), end-user credentials, and the Host
...
export function sensitiveHeaders(headers) {
  return Object.keys(headers).filter((name) => {
    ...
    return SENSITIVE_HEADER_NAMES.has(lower) || lower.startsWith("x-enclavia-");
  });
}
